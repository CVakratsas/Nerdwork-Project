...
if($_SERVER["REQUEST_METHOD"] == "GET"){
    $sql = "SELECT id, name, phone, email, profilePhoto, office, rating FROM professors";
    if($stmt = mysqli_prepare($link, $sql)){
        if(mysqli_stmt_execute($stmt)){
                // Store result
                mysqli_stmt_store_result($stmt);
                mysqli_stmt_bind_result($stmt, $id, $name, $phone, $email, $profilePhoto, $office, $rating);
                ...
                while(mysqli_stmt_fetch($stmt)){
                    ...
                    $jsonResult->triggerResults[$index]->office = $office;
                    $jsonResult->triggerResults[$index]->rating = $rating;
                    $jsonResult->triggerResults[$index]->bio = getProfessorBio($id);
                    $index = $index+1;
                }
                header('Content-type: application/json; charset=utf-8');
