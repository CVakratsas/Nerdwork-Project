...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $sql = "SELECT id, username, password, bSpecialAccess, accountType, orientation, email, displayName, associatedProfessor FROM users WHERE username = ?";
        ...
        if($stmt = mysqli_prepare($link, $sql)){
            ...
            if(mysqli_stmt_execute($stmt)){
                ...
                if(mysqli_stmt_num_rows($stmt) == 1){                    
                    ...
                    mysqli_stmt_bind_result($stmt, $id, $username, $hashed_password, $special, $type, $orientation, $email, $displayName, $associatedProfessor);
                    if(mysqli_stmt_fetch($stmt)){
                        if(password_verify($data["password"], $hashed_password)){
                            ...
                            $jsonResult->triggerResults->orientation = $orientation;
